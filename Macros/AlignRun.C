...
// Fits a histogram (WorkHistogram) to another histogram (SeedHistogram).
//
// Example :
//    TAlignHistogram AlignHistogram( SeedHistogram, WorkHistogram, Channel );
//    AlignHistogram.Align();
//    // AlignHistogram.EportENGInfoToChannel(); // Export to current TChannel
//
...
// Function used to turn the work histogram into a function. The parameters are used
// to stretch, shift, and grow the work histogram to match the seed histogram.
...
void TAlignHistogram::Align(Double_t XLow, Double_t XHigh)
{
   ...
   // Fit work histogram to the seed histogram
...
// Gererates the seed histogram a special root file. Needs to be called before AlignTree.
...
// Aligns the TTree data to the seed histogram. Useful for compensating for drift between detector runs.
// Automaticaly exports newly calculated coefficents to the files TChannel
void AlignTree(TTree *AnalysisTree)
{
   ...
   for (int i = 0; i <= NChans; ++i) {
      ...
      if (pChannel == nullptr || SeedSlice->Integral() < 2000 || WorkSlice->Integral() < 2000) continue;
