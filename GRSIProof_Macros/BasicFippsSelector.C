...
void BasicFippsSelector::CreateHistograms()
{
   ...
   fHSparse["ggE"]    = new THnSparseF("ggE", "#gamma-#gamma Coincidence", 2, ggBins, ggXMin, ggXMax);
   fHSparse["aaE"]    = new THnSparseF("aaE", "#Addback-Addback Coincidence", 2, ggBins, ggXMin, ggXMax);
   fHSparse["ggEb"]   = new THnSparseF("ggEb", "#gamma-#gamma Background Coincidence", 2, ggBins, ggXMin, ggXMax);
   fHSparse["ggEbs"] =
      new THnSparseF("ggEbs", "#gamma-#gamma Coincidence Background Subtracted", 2, ggBins, ggXMin, ggXMax);

   Int_t    gggBins[3] = {10000, 10000, 10000};
   Double_t gggXMin[3] = {0., 0., 0.};
   Double_t gggXMax[3] = {10000., 10000., 10000.};
   fHSparse["gggE"]    = new THnSparseF("gggE", "#gamma-#gamma-#gamma Coincidence", 3, gggBins, gggXMin, gggXMax);
   fHSparse["aaaE"]    = new THnSparseF("aaE", "Addback Coincidence Cube", 3, gggBins, gggXMin, gggXMax);
...
void BasicFippsSelector::FillGammaHistograms()
{
   ...
   for (auto i = 0; i < fFipps->GetMultiplicity(); ++i) {
      ...
      fH2.at("gEMat")->Fill(Fipps1->GetChannel()->GetNumber(), Fipps1->GetEnergy());
      ...
      for (auto j = 0; j < fFipps->GetMultiplicity(); ++j) {
         ...
         for (auto k = 0; k < fFipps->GetMultiplicity(); ++k) {
            ...
            if (IsCoincidencePrompt(Fipps1, Fipps2, Fipps3)) {
               Double_t gggEnergy[3] = {Fipps1->GetEnergy(), Fipps2->GetEnergy(), Fipps3->GetEnergy()};
               fHSparse.at("gggE")->Fill(gggEnergy);
            }
         }
      }
   }
   // Loop over all LaBr Hits
   for (auto i = 0; i < fFippsLaBr->GetMultiplicity(); ++i) {
      auto FippsLaBr1 = fFippsLaBr->GetHit(i);
      fH2.at("gEMat")->Fill(FippsLaBr1->GetChannel()->GetNumber(), FippsLaBr1->GetEnergy());
   }
}
...
void BasicFippsSelector::FillSuppressedAddbackHistograms()
{
   ...
   for (auto i = 0; i < fFipps->GetSuppressedAddbackMultiplicity(fFippsBgo); ++i) {
...
void BasicFippsSelector::EndOfSort()
{
   ...
   fHSparse.at("ggEbs")->Add(fHSparse.at("ggEb"), -1);
}
